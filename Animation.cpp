...
int Animation::Save(const wstring& _RelativePath)
{
...
int Animation::Load(const wstring& _RelativePath)
{
    ...
    while (true)
    {
        ...
        else if (strRead == L"[INDEX]")
        {
            ...
            m_Sprites[index] = AssetMGR::GetInstance()->LoadSprite(SpriteKey, SpritePath);
        }
    }
