...
{
	if (nullptr == SoundMGR::GetInstance()->GetSoundDevice())
		assert(nullptr);
...
bool Sound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(SoundMGR::GetInstance()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
...
{
	SoundMGR::GetInstance()->RegisterToBGM(this);
